  ...
  container.innerHTML = `
    <div class="admin-section-card">
      <div class="admin-section-card__header">
        <h3 class="admin-section-card__title">Skills (${skills.length})</h3>
...
function filterAdminSkills(cat, btn) {
  ...
  const rows = document.querySelectorAll('#admin-skills-grid tr[data-category]');
  rows.forEach(row => {
...
function openSkillModal(editIndex = null) {
  ...
  const skill = isEdit ? data.skills[editIndex] : {};
  ...
  const formHTML = `
    <div class="admin-form">
      <div class="form-group">
        ...
        <input type="text" class="form-input" id="skill-name" value="${AdminUI.escapeHtml(skill.name || '')}" />
      </div>
      <div class="admin-form__row">
        ...
        <div class="form-group" id="skill-new-cat-wrap" style="display:none;">
          <label class="form-label">New Category</label>
          <input type="text" class="form-input" id="skill-new-cat" placeholder="e.g., DevOps" />
        </div>
      </div>
      <div class="form-group">
        <label class="form-label">Icon Key <small style="color:var(--text-muted);">(devicons name, e.g., 'react', 'nodejs')</small></label>
        <input type="text" class="form-input" id="skill-icon" value="${AdminUI.escapeHtml(skill.icon || '')}" />
      </div>
      <div class="form-group">
        <label class="form-label">Short Description</label>
        <input type="text" class="form-input" id="skill-short" value="${AdminUI.escapeHtml(skill.shortDesc || '')}" />
      </div>
      <div class="form-group">
        <label class="form-label">Full Description</label>
        <textarea class="form-textarea" id="skill-desc" rows="3">${AdminUI.escapeHtml(skill.description || '')}</textarea>
      </div>
      <div class="form-group">
        <label class="form-label">Proficiency: <strong id="skill-prof-label">${skill.proficiency || 50}%</strong></label>
        <input type="range" id="skill-proficiency" min="0" max="100" value="${skill.proficiency || 50}" style="width:100%;accent-color:var(--accent);" />
      </div>
      <div class="form-group">
        <label class="form-label">Related Projects</label>
        <div id="skill-projects-tags"></div>
      </div>
      <div class="admin-form__actions">
        <button class="btn btn--primary" id="skill-save">${isEdit ? 'Update' : 'Add'} Skill</button>
        <button class="btn btn--secondary" id="skill-cancel">Cancel</button>
      </div>
    </div>`;
  ...
  const newCatWrap = document.getElementById('skill-new-cat-wrap');
  catSelect.addEventListener('change', () => {
    newCatWrap.style.display = catSelect.value === '__new' ? '' : 'none';
  });

  // Proficiency slider label
  const slider = document.getElementById('skill-proficiency');
  const label = document.getElementById('skill-prof-label');
  slider.addEventListener('input', () => { label.textContent = slider.value + '%'; });

  // Related projects tag input
  AdminUI.createTagInput(document.getElementById('skill-projects-tags'), [...(skill.relatedProjects || [])]);
  ...
  document.getElementById('skill-save').addEventListener('click', () => {
    ...
    if (category === '__new') {
      category = document.getElementById('skill-new-cat').value.trim();
      if (!category) { AdminUI.toast('Enter a category name', 'error'); return; }
    }
    ...
    const newSkill = {
      ...
      shortDesc: document.getElementById('skill-short').value.trim(),
      description: document.getElementById('skill-desc').value.trim(),
      proficiency: parseInt(slider.value),
      relatedProjects: relProjects,
      gallery: skill.gallery || [],
    };

    const d = DataStore.getData();
    if (isEdit) { d.skills[editIndex] = newSkill; } else { d.skills.push(newSkill); }
    DataStore.saveAll(d);
...
function deleteSkill(index) {
  ...
  AdminUI.confirm('Delete Skill', `Remove "${skill.name}"? This cannot be undone.`, () => {
