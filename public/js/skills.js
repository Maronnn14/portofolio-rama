...
function openSkillModal(skillId) {
  ...
  const rp = (s.relatedProjects||[]).map(pid => PortfolioData.projects.find(p => String(p.id)===String(pid))).filter(Boolean);
  content.innerHTML = `<button class="skill-modal__close" onclick="closeSkillModal()">&times;</button><div class="skill-modal__header"><div class="skill-modal__icon"><img src="${skillIconMap[s.icon]||''}" alt="${s.name}" /></div><div><h2 class="skill-modal__title">${s.name}</h2><span class="tag tag--accent">${s.category}</span></div></div><div class="skill-modal__section"><h3 class="skill-modal__section-title">Description</h3><p class="skill-modal__desc">${s.description}</p></div><div class="skill-modal__section"><h3 class="skill-modal__section-title">Proficiency</h3><div class="proficiency-bar"><div class="proficiency-bar__fill" id="proficiency-fill" style="width:0%"></div></div><div class="proficiency-info"><span class="proficiency-info__level">${s.level}</span><span class="proficiency-info__percent">${s.proficiency}%</span></div></div>${rp.length?`<div class="skill-modal__section"><h3 class="skill-modal__section-title">Related Projects</h3><div class="skill-modal__projects">${rp.map(p=>`<a href="project-detail.html?id=${p.id}" class="skill-modal__project-link"><div><div class="skill-modal__project-name">${p.name}</div><div class="skill-modal__project-desc">${p.shortDesc}</div></div></a>`).join('')}</div></div>`:''}${s.gallery&&s.gallery.length?`<div class="skill-modal__section"><h3 class="skill-modal__section-title">Gallery</h3><div class="skill-modal__gallery" id="skill-gallery-${s.id}">${s.gallery.map((img,i)=>`<div class="skill-modal__gallery-item" data-lightbox="${img}"><img src="${img}" alt="${s.name} ${i+1}" loading="lazy" /></div>`).join('')}</div></div>`:''}`;
  modal.classList.add('open'); document.body.style.overflow='hidden';
  setTimeout(() => { const f=document.getElementById('proficiency-fill'); if(f) f.style.width=s.proficiency+'%'; }, 300);
  if(s.gallery&&s.gallery.length) { const gi=content.querySelectorAll(`#skill-gallery-${s.id} .skill-modal__gallery-item`); const gimg=s.gallery.map(src=>({src,alt:s.name})); gi.forEach((item,idx)=>{ item.addEventListener('click',()=>{ lightbox.open(gimg,idx); }); }); }
}
