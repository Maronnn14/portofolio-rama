...
const DEFAULT_DATA = (
  ...
  && window.PORTFOLIO_DB_DATA
  && Object.keys(window.PORTFOLIO_DB_DATA).length
)
  ? window.PORTFOLIO_DB_DATA
  : DEFAULT_DATA_FALLBACK;

async function syncPortfolioDataToDatabase(data) {
  if (typeof window === 'undefined' || !window.PORTFOLIO_API_BASE) return;

  try {
    await fetch(`${window.PORTFOLIO_API_BASE}/portfolio-data`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
      body: JSON.stringify({ data }),
    });
  } catch (e) {
    console.warn('DataStore: Failed to sync to database', e);
  }
}
...
const DataStore = {
  ...
  _persist() {
    try {
      ...
      if (typeof window !== 'undefined' && typeof window.syncPortfolioDataToDatabase === 'function') {
        window.syncPortfolioDataToDatabase(this._cache);
      }
    } catch (e) {
